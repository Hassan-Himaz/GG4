...
def stochastic_subspace_identification(
    ...
    """Estimate A and C from output data via SSI-COV (covariance-driven).

    Builds the block Hankel matrix of output correlations R_j = (1/T) Y[:,j:] Y[:,:-j].T,
    then SVD → observability matrix → A via shift invariance, C from first block row.
    ...
    # ── Step 1: estimate output correlation sequence R_0 .. R_{2i-1} ─────────
    # R_j = (1/T) Σ_t y_{t+j} y_t^T   shape (m, m) each
    # Centre the data first
    Y = Y - Y.mean(axis=1, keepdims=True)

    def correlation(lag: int) -> np.ndarray:
        if lag == 0:
            return (Y @ Y.T) / T
        return (Y[:, lag:] @ Y[:, :-lag].T) / (T - lag)

    # ── Step 2: build block Hankel matrix of correlations ─────────────────────
    # H = [[R_1,   R_2,   ..., R_{i+1} ],
    #      [R_2,   R_3,   ..., R_{i+2} ],
    #      ...
    #      [R_{i}, R_{i+1},..., R_{2i}  ]]
    ...
    for row in range(i):
        for col in range(i):
            lag = row + col + 1          # R_{row+col+1}
            H[row * m:(row + 1) * m,
              col * m:(col + 1) * m] = correlation(lag)

    # ── Step 3: SVD of H → observability × controllability ───────────────────
    # H = U S V^T ≈ Γ (Δ^T)  where Γ is the observability matrix
    U, s, Vt = svd(H, full_matrices=False)
    U1 = U[:, :n]                        # (i*m, n)
    S1 = np.diag(s[:n])                  # (n,   n)
    Γ  = U1 @ np.sqrt(S1)               # (i*m, n)  observability matrix

    # Normalise each column of Γ to unit norm
    col_norms = np.linalg.norm(Γ, axis=0, keepdims=True)
    col_norms = np.maximum(col_norms, 1e-10)   # avoid divide by zero
    Γ = Γ / col_norms    

    # ── Step 4: C from first block row of Γ ───────────────────────────────────
    C = Γ[:m, :]                         # (m, n)

    # ── Step 5: A via shift invariance  Γ_{1..i-1} A = Γ_{2..i} ─────────────
    Γ_up   = Γ[:(i - 1) * m, :]         # ((i-1)m, n)
    Γ_down = Γ[m:,           :]         # ((i-1)m, n)
    A, _, _, _ = lstsq(Γ_up, Γ_down, rcond=None)

    # ── Step 6: enforce stability ─────────────────────────────────────────────
    rho = np.abs(np.linalg.eigvals(A)).max()
    if rho >= 0.99:
        A = A * (0.99 / rho)
...
def initialise_B(
    ...
    Q = np.eye(n) * y_var * 0.1   # process noise = 10% of observation noise scale
...
def fit(
    ...
    # Diagnostic: check residual scale before input estimation
...
def festimate_latent_and_input(
    ...
    Wraps the full pipeline with the standard estimator signature.

    Parameters
    ----------
    observation  : (T, m)  observed emissions
    LatentDim    : n       latent state dimension
    ...
    Y    = observation.T          # pipeline expects (m, T)

    # Auto horizon: needs to see enough of the dynamics but bounded by T
    if ssi_i is None:
    ...
    latent_states = final_res.x_smooth          # (T, LatentDim)
    inputs        = U.T                         # (T, InputDim)  U is (p, T-1)

    # Pad inputs by one timestep to match observation length
    inputs = np.vstack([inputs, inputs[-1:, :]])   # (T, InputDim)
